...
def Lire_Echelle_Mallat2D(wc,l,ind):
    
    n = wc.shape[0]
    m = wc.shape[1]
    if ind==1:
        img=wc[0:int(n/(2**l)),int(m/(2**l)):int(m/(2**(l-1)))]
    if ind==2:
        img=wc[int(n/(2**l)):int(n/(2**(l-1))),0:int(m/(2**l))]
    if ind==3:
        img=wc[int(n/(2**l)):int(n/(2**(l-1))),int(m/(2**l)):int(m/(2**(l-1)))]
    if ind==0:
        img=wc[0:int(n/(2**(l))),0:int(m/(2**(l)))]
    return img

def Ecrire_Echelle_Mallat2D(wc,img,l,ind):
